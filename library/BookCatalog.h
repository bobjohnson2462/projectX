...
public:
    ...
    // Если книга с теми же (author, title, year, genre) уже существует — увеличивает количество экземпляров.
    // Запрещено: отрицательное/нулевое количество экземпляров.
    // Возвращает id книги (существующей или новой). Если ошибка — возвращает -1.
    int addOrMergeBook(const std::string& author,
        ...
        if (copiesToAdd <= 0) {
            if (error) *error = "Нельзя добавлять 0 или отрицательное число экземпляров.";
            return -1;
        }
        if (year <= 0) {
            if (error) *error = "Год издания должен быть положительным.";
            return -1;
        }
...
private:
    ...
    static bool contains(const std::string& text, const std::string& sub, bool ignoreCaseAscii) {
        ...
        // Примечание: работает надёжно для латиницы/ASCII; для кириллицы регистр лучше вводить одинаково.
